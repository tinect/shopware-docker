        ...
        for d in ${CODE_DIRECTORY}/* ; do
            if [[ -d "$d" ]]; then
                ...
                if [[ -f "$d/public/index.php" ]]; then
                    echo "      ${NAME}.sw6.tinect.de: 127.0.0.1" >> ${DOCKER_OVERRIDE_FILE}
                else
                    echo "      ${NAME}.sw5.tinect.de: 127.0.0.1" >> ${DOCKER_OVERRIDE_FILE}
                fi
            fi
        done
    ...
    if [[ ${CODE_FOLDER_CONTENT} ]]; then
        ...
        for d in ${CODE_DIRECTORY}/* ; do
            if [[ -d "$d" ]]; then
                NAME=$(basename $d)
                if [[ -f "$d/public/index.php" ]]; then
                    echo "      - nginx:${NAME}.sw6.tinect.de" >> ${DOCKER_OVERRIDE_FILE}
                else
                    echo "      - nginx:${NAME}.sw5.tinect.de" >> ${DOCKER_OVERRIDE_FILE}
                fi
            fi
        done
        if [[ ${Platform} != "Linux" ]]; then
